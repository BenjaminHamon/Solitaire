...
class UnityEditorClient:
    ...
    async def run(self,
        ...
        await self._run_internal(unity_arguments, custom_arguments, log_file_path = log_file_path, simulate = simulate)
    ...
    async def reimport(self,
        ...
        try:
            await self._run_internal(unity_arguments, log_file_path = log_file_path, simulate = simulate)
    ...
    async def execute(self, # pylint: disable = too-many-arguments
        ...
        logger.info("Executing '%s' (Project: '%s')", method, self._project_path)
    ...
    async def _run_internal(self,
        ...
        try:
            if not simulate:
                try:
                    await self._process_runner.run(command_as_object, process_options, output_handlers, check_exit_code = True)
                finally:
